...
my $scriptpath = "/opt/exp_soft/galaxy-tools/custom_tools/bin";
...
#			print $readid."\n";
	...
	my $opttoadd = '-nqvalue='.$option{nqvalue}.' -nqsmq='.$option{nqsmq}.' -nqsaq='.$option{nqsaq}.' -nqssize='.$option{nqssize};

	if($turn == 1){
		print "IE COUNT : $iecount\n";
		system("perl ".$scriptpath."runMosaik2.pl -fa $output"."_ie.fasta -qual $output"."_ie.qual -ref $reffile -o $output"."_ie -qlx -param454 $opttoadd");
	}elsif($turn == 2){
		system("perl ".$scriptpath."runMosaik2.pl -fa $tempfasta -qual $tempqual -ref $reffile -o $tempqln -gop=30 -qlx -st 454 -gep 6.66 -hgop 4 $opttoadd");
		if($option{noorf})
		{
			if($option{details})
			{
...
system("perl ".$scriptpath."runMosaik2.pl -fa $output"."_cleaned.fasta -qual $output"."_cleaned.qual -ref $reffile -o $output"."_final -qlx -gop=30 -st 454 -gep 6.66 -hgop 4 $opttoadd");
...
sub cleanFiles
{
	...
	unless($option{details})
	{
		...
		unless($option{noorf})
		{
			...
			#system("rm $output"."_homo.multiple.bam");
			system("rm $output"."_homo_unmappedIDs.txt");
		...
		#system("rm $output"."_ie.multiple.bam");
		system("rm $output"."_ie_unmappedIDs.txt");
