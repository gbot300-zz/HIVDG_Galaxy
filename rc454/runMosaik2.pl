...
my $mosaikpath = "/opt/exp_soft/galaxy-tools/custom_tools/bin";
my $samtoolspath = "";
my $mosaiknetworkpath = "/opt/exp_soft/MOSAIK-2.0/src/networkFile/";
...
if($option{qlx}){
	system("perl ".$scriptpath."samToQlx.pl $output.sam -ref ".$option{ref}." $output  -nqssize ".$option{nqssize}." -nqvalue ".$option{nqvalue}.$sam2qlxopt);
}
#if($option{qlxonly})
#{
